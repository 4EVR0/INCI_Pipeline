...
class CosIngQuerySplitter:
    def __init__(
        ...
        self.seed_chars = seed_chars or settings.seed_chars
        self.next_chars = next_chars or settings.next_chars
        self.safe_limit = safe_limit or settings.safe_limit
        ...
        if self.cache_path and self.cache_path.exists():
            ...
            with open(self.cache_path, "r", encoding="utf-8") as f:
                self.count_cache = json.load(f)
            self.logger.info("[CACHE_LOADED] entries=%s path=%s", len(self.count_cache), self.cache_path)
    ...
    def save_cache(self):
        if self.cache_path:
            ...
            with open(self.cache_path, "w", encoding="utf-8") as f:
                json.dump(self.count_cache, f, ensure_ascii=False, indent=2)
            self.logger.info("[CACHE_SAVED] entries=%s path=%s", len(self.count_cache), self.cache_path)
    ...
    def count_results(self, query: str) -> int:
        ...
        if cache_key in self.count_cache:
            self.logger.info("[CACHE_HIT] query=%s total=%s", query, self.count_cache[cache_key])
    ...
    def expand_query(
        ...
        for ch in self.next_chars:
            ...
            self.logger.info("[COUNT] query=%s total=%s", q, count)
            ...
            if count <= self.safe_limit:
                ...
                continue
            if depth >= max_depth:
                self.logger.warning("[OVERSIZED_AT_MAX_DEPTH] query=%s total=%s max_depth=%s", q, count, max_depth)
    ...
    def build_queries(
        ...
        for seed in seeds:
            q = f"{seed}*"
            count = self.count_results(q)
            self.logger.info("[SEED] query=%s total=%s", q, count)
            ...
            sub_final, sub_oversized = self.expand_query(prefix=seed, depth=1, max_depth=max_depth)
            final_queries.extend(sub_final)
            oversized_queries.extend(sub_oversized)

        final_queries = sorted(set(final_queries))
        oversized_queries = sorted(set(oversized_queries), key=lambda x: (x[0], x[1]))
        self.logger.info("[QUERY_BUILD_DONE] final_query_count=%s", len(final_queries))
        self.logger.info("[QUERY_BUILD_DONE] oversized_query_count=%s", len(oversized_queries))
        return final_queries, oversized_queries
